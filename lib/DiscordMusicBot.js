...
class DiscordMusicBot extends Client {
  ...
  build() {
    ...
    if (this.config.debug === true) {
      this.warn("Đang bật Debug mode!");
      this.warn("Chỉ bật khi bạn biết bạn là ai!");
    ...
    this.manager = new Manager({
      ...
      clientName: `minibo0/v${require("../package.json").version} (Bot: ${
      ...
      .on("nodeConnect", (node) =>
        this.log(
          `Node: ${node.options.identifier} | Đã kết nối thành công tới node Lavalink.`
        )
      )
      .on("nodeReconnect", (node) =>
        this.warn(
          `Node: ${node.options.identifier} | Đang thử kết nối lại node Lavalink.`
        )
      )
      .on("nodeDestroy", (node) =>
        this.warn(
          `Node: ${node.options.identifier} | Lavalink node đã hỏng.`
        )
      )
      .on("nodeDisconnect", (node) =>
        this.warn(
          `Node: ${node.options.identifier} | Đã bị ngắt kết nối khỏi node Lavalink.`
        )
      )
      .on("nodeError", (node, err) => {
        this.warn(
          `Node: ${node.options.identifier} | Node Lavalink xảy ra lỗi: ${err.message}.`
        );
      })
      // on track error warn and create embed
      .on("trackError", (player, err) => {
        this.warn(
          `Player: ${player.options.guild} | Track xảy ra lỗi: ${err.message}.`
        );
        //console.log(err);
        ...
        let errorEmbed = new MessageEmbed()
          .setColor("RED")
          .setTitle("Có lỗi xảy ra khi phát!")
          .setDescription(`Không thể tải bài hát: \`${title}\``)
          .setFooter({
            text: "Có lỗi xảy ra nhưng không phải lỗi của bạn đâu xD",
          });
      ...
      .on("trackStuck", (player, err) => {
        this.warn(`Track xảy ra lỗi: ${err.message}`);
        //console.log(err);
        let song = player.queue.current;
        var title = escapeMarkdown(song.title)
        var title = title.replace(/\]/g,"")
      ...
      .on("playerDestroy", (player) => {
        this.warn(
          `User: ${player.options.guild} | Đã huỷ phát lại nhạc ở trong ${client.guilds.cache.get(player.options.guild)
              ? client.guilds.cache.get(player.options.guild).name
              : "guild"
          }`
        )
      ...
      .on("loadFailed", (node, type, error) =>
        this.warn(
          `Node: ${node.options.identifier} | Thất bại khi tải ${type}: ${error.message}`
        )
      )
      ...
      .on(
        ...
        /** @param {EpicPlayer} player */ async (player, track) => {
          ...
          this.warn(
            `User: ${
              player.options.guild
            } | Đang phát bài hát [${colors.blue(track.title)}]`
          );
        ...
        /** @param {EpicPlayer} */ async (player, track) => {
          ...
          if (autoQueue) {
            ...
            let queueEmbed = new MessageEmbed()
              .setColor(client.config.embedColor)
              .setAuthor({
                name: "Hàng chờ đã kết thúc.",
                iconURL: client.config.iconURL,
              })
              .setFooter({ text: "Hàng chờ đã kết thúc" })
              .setTimestamp();
