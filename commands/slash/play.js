...
const command = new SlashCommand()
  .setName("play")
  .setDescription(
    "Tìm kiếm và phát nhạc đa nền tảng. Hỗ trợ YouTube, Spotify, Deezer, Apple Music,..."
  )
  .addStringOption((option) =>
    option
      .setName("song")
      .setDescription("Nhập tên bài hát hoặc đường dẫn cụ thể")
  ...
  .setRun(async (client, interaction, options) => {
    ...
    if (!node) {
      return interaction.reply({
        embeds: [client.ErrorEmbed("Chưa kết nối với server Lavalink!")],
      });
    }
    ...
    let query = options.getString("song", true);
      ...
      await interaction
        .editReply({
          embeds: [
            new MessageEmbed()
              .setColor("RED")
              .setDescription("Không tìm thấy bài hát nào như vậy cả."),
          ],
        })
        .catch(this.warn);
    }
    ...
    if (res.loadType === "TRACK_LOADED" || res.loadType === "SEARCH_RESULT") {
      ...
      let addQueueEmbed = new MessageEmbed()
        .setColor(client.config.embedColor)
        .setAuthor({ name: "Đã thêm bài hát vào hàng chờ", iconURL: client.config.iconURL })
